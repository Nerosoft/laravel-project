...
class ReceptionController extends PatientInfo implements LangObject
{
    ...
    public function __construct(){
        ...
        if(Route::currentRouteName() === 'createPatientServices' || Route::currentRouteName() === 'editPatientServices'){
            $this->roll = [
                ...
                'patentCode' => ['required', Rule::in(isset($this->ob['Patent'])?array_keys($this->ob['Patent']):null)],
                'know' => ['required', Rule::in(isset($this->ob['Knows'])?array_keys($this->ob['Knows']):null)],
                'discount' => ['required', 'numeric', 'min:0'],
                'delayedMoney' => ['required', 'numeric', 'min:0'],
                'paymentDate' => ['required', 'date'],
                'paymentAmount' => ['required', 'numeric', 'min:0'],
                'paymentMethod' => ['required', Rule::in(array_keys($this->ob[$this->ob['Setting']['Language']]['PaymentMethodBox']))],
            ];
            $this->message = [
                'item.required'=>$this->error5,
                'item.array'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationItemInvalid'],
                'patentCode.required'=>$this->error3,
                'patentCode.in'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationPatentCodeInvalid'],
                'know.required'=>$this->error4,
                'know.in'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationKnowInvalid'],
                'discount.required'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationDiscountRequired'],
                'discount.numeric'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationDiscountInvalid'],
                'discount.min'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationDiscountInvalid'],
                'delayedMoney.required'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationDelayedMoneyRequired'],
                'delayedMoney.numeric'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationDelayedMoneyInvalid'],
                'delayedMoney.min'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationDelayedMoneyInvalid'],
                'paymentDate.required'=>$this->error6,
                'paymentDate.date'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationPaymentDateInvalid'],
                'paymentAmount.required'=>$this->error7,
                'paymentAmount.numeric'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationPaymentAmountInvalid'],
                'paymentAmount.min'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationPaymentAmountInvalid'],
                'paymentMethod.required'=>$this->error2,
                'paymentMethod.in'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationPaymentMethodInvalid'],
                'item.*.required'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationItemInvalid']
            ];
        }else{
            ...
            parent::__construct('Receipt', $this->ob, $this->ob['Receipt']?Receipt::fromArray2(array_reverse($this->ob['Receipt']), $this->myPatent, $this->arr1, $this->ob[$this->ob['Setting']['Language']]['SelectTestBox'], $this->ob[$this->ob['Setting']['Language']]['PaymentMethodBox']):array());
            ...
            $this->arr1 = isset($this->ob['Knows']) ? MyKnows::fromArray($this->ob['Knows']):array();
            $this->successfully1 = $this->ob[$this->language]['Receipt']['LoadMessage'];
        }
    }
    ...
    public function makeAddReceipt(){
        ...
        return response()->json([
            'success' => true,
            'message'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationAdd']
        ]);
    }
    public function makeEditReceipt(){
        $this->roll['id'] = ['required', Rule::in($this->ob['Receipt']?array_keys($this->ob['Receipt']):null)];
        $this->getEditDataBase($this->ob, 'Receipt', $this);
        return response()->json([
            'success' => true,
            'message'=>$this->ob[$this->ob['Setting']['Language']]['Receipt']['PatientRegisterationEdit']
        ]);
    }

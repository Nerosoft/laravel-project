...
class ChangeLanguageController extends Page implements ActionInit2, ValidRull
{
    ...
    public function initValidRull(){
        array_push($this->roll['id'], Rule::in(Route::currentRouteName() === 'branchMain'?(Rays::find(request()->session()->get('userLogout'))['Branch']?array_merge([request()->session()->get('userLogout')],array_keys(Rays::find(request()->session()->get('userLogout'))['Branch'])):request()->session()->get('userLogout')):array_keys($this->ob[$this->ob['Setting']['Language']]['AllNamesLanguage'])));

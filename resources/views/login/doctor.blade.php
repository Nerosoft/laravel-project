...
<link rel="stylesheet" href="{{asset('css/login/doctor-login.css')}}">
@section('containt')
<div class="container">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="myForm" action="{{route('language.changeLanguage','language_doctor')}}" method="POST" onsubmit="return validateLanguage('{{ $userLanguage }}')">
        @csrf
...
<div class="user-login">
        <h4>{{$lang->label3}}</h4>
        <form action="{{route('loginDoctor')}}" method="POST" onsubmit="return validateForm()">
        @csrf
            <input type="hidden" name="userId" value = "{{$lang->RaysId}}">
...
<script src="{{asset('js/login/doctor-login.js')}}" type="text/javascript"></script>
@endsection